...
class TensorDiffusionModel(nn.Module):
   def __init__(self, tensor_shape=(51, 51, 25), t_emb_dim=64, rank=250):
       super().__init__()
       self.tensor_shape = tensor_shape  
       #(51, 51, 25) for calorimeterr or (64, 64, 3) for celeba data
       self.t_emb_dim = t_emb_dim
       self.rank = rank

       self.flat_dim = np.prod(tensor_shape)
       self.shape0, self.shape1, self.shape2 = tensor_shape

       #Total parameters (dim0 + dim1 + dim2) * rank
       total_factor_params = (self.shape0 + self.shape1 + self.shape2) * rank
       ...
       self.denoiser = nn.Sequential(
          ...
          nn.LeakyReLU(0.01),
          nn.Dropout(0.2),
          nn.Linear(512, total_factor_params)
   ...
   def forward(self, x, t):
       ...
       t_embed = get_timestep_embedding(t.view(-1), self.t_emb_dim)
       x_input = torch.cat([x_flat, t_embed], dim=1)

       out = self.denoiser(x_input)
       r = self.rank

       a_end = self.shape0 * r
       b_end = a_end + self.shape1 * r
       c_end = b_end + self.shape2 * r

       a = out[:, :a_end].view(-1, self.shape0, r)
       b = out[:, a_end:b_end].view(-1, self.shape1, r)
       c = out[:, b_end:c_end].view(-1, self.shape2, r)
...
def reconstruct_tensor(a, b, c):
    ...
    for i in range(batch_size):
        tensor = tl.cp_to_tensor((None, [a[i], b[i], c[i]]))
...
def train_tensor_diffusion(model, tensor_data, batch_size=32, lr=1e-3, epochs=100, T=1000, device='cuda:1'):
   ...
   tensors = torch.stack([t for t in tensor_data]).to(device)
   ...
   for epoch in range(epochs):
       ...
       for i in range(0, tensors.size(0), batch_size):
           ...
           a, b, c = model(noisy, t)
           recon = reconstruct_tensor(a, b, c)
           ...
           del batch, t_idx, t, noisy, a, b, c, recon, loss
...
def sample_tensors(model, num_samples, T=100, device='cuda:1'):
   with torch.no_grad():
       ...
       for i in reversed(range(1, T)):
           ...
           t_tensor = torch.full((num_samples,), i, dtype=torch.long, device=device)

           a, b, c = model(x, t_tensor)
           x0_pred = reconstruct_tensor(a, b, c)
           x = ddim_step(x, x0_pred, alpha_t, alpha_prev)

       return torch.clamp(x0_pred, min = 0)