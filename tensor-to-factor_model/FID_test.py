...
for rank in ranks:
    model, original_tensors, ecal_train, ecal_test, parameters = run_model(rank=rank, num_samples=num_samples, epochs=50)
    generated_tensors = generate_tensors(model=model, num_generated=num_generated, rank=rank)
    save_directory = f"generated_tensors2_{rank}"; os.makedirs(save_directory, exist_ok=True)
