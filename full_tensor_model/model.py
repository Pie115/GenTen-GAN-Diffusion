...
def train_tensor_diffusion(model, tensor_data, batch_size=32, lr=1e-3, epochs=100, T=1000, device='cuda:0'):
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, tensors.size(0), batch_size):
            ...
            noisy = add_scaled_noise(batch, t)
            pred = model(noisy, t)
...
def sample_tensors(model, num_samples, tensor_shape=(51, 51, 25), T=100, device='cuda:0'):
    model.eval()
    alpha_bars = make_ddim_schedule(T).to(device)
