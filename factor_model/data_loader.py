...
def extract_ecal(file_amount = 1, device = 'cuda:1', num_samples = 1000):
    ...
    ecals_subset = torch.tensor(ecal_data[:num_samples], dtype=torch.float32, device=device)
...
def save_factors_to_cache(factors, index_map, valid_indices, rank, cache_dir="cached_factors"):
...
def load_factors_from_cache(rank, cache_dir="cached_factors", device='cuda:1'):
    path = os.path.join(cache_dir, f"rank_{rank}_factors.pkl")
